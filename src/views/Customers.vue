<template>
  ...
  <div v-else class="p-6 max-w-[1300px] mt-[100px] mx-auto">
    <h1 class="text-3xl font-semibold mb-4">Customers</h1>
    ...
    >
      <div
        class="bg-white p-6 rounded-2xl shadow-xl max-w-md w-full transform scale-95 border border-gray-200"
      >
  ...
  setup() {
    ...
    const fetchCustomers = async (page = 1) => {
      ...
      try {
        const { data } = await axios.get(
          `http://localhost:8000/api/customers?page=${page}&per_page=${pagination.value.per_page}`
        );
    ...
    const saveCustomer = async () => {
      ...
      try {
        if (form.value.id) {
          await axios.put(
            `http://localhost:8000/api/customers/${form.value.id}`,
            form.value
          );
    ...
    const deleteCustomer = async () => {
      ...
      try {
        await axios.delete(
          `http://localhost:8000/api/customers/${deleteId.value}`
        );
